...
{
    public class GHComponent_SimpleGrid : GH_Component
    {
        ...
        // CONSTRUCTOR
        public GHComponent_SimpleGrid()
            : base("Simple Grid", "Simple Grid", "Relaxation of a grid on fixed supports", "Marsupilami", "Example")
        {
        ...
        {
            get { return new Guid("{244d1735-fd81-44be-97ab-4323359e6381}"); }
        }
        ...
        private void Init()
        {
            ...
            for (int j = nH; j < nH+nV; j++)
            {
                ...
                for (int k = 1; k < n; k++)
                {
                    //xxi = (Vector3d)polylines_V[j - nH][k];
                    //Xi[2 * k - 1] = (xxi + Xi[2 * k - 2])/2;
                    //Xi[2 * k] = xxi;
                    //Fext[2 * k - 1] = new Vector3d(0, 0, fz);
                    //Fext[2 * k] = new Vector3d(0, 0, fz);
                ...
                for (int j = 0; j < nV; j++)
                {
                    for (int ki = 0; ki < polylines_H[i].Count; ki++)
                    {
                        for (int kj = 0; kj < polylines_V[j].Count; kj++)
                        {
                            ...
                            {
                                //constraints.Add(DRLinkConstraint.CreateSingleJointConstraint(i, 2*ki, j+nH, 2*kj));
            ...
            for (int i = 0; i < nH; i++)
            {
                for (int ki = 0; ki < polylines_H[i].Count; ki++)
                {
                    for (int j = 0; j < pts.Count; j++)
			        {
                        ...
                        {
                            //Vector3d vj = new Vector3d(pts[j].X, pts[j].Y, pts[j].Z);
                            //constraints.Add(DRMechanicalConstraint.CreateSurfaceAttractorConstraint(i,ki,srf,k[j]));
                            //constraints.Add(DRMechanicalConstraint.CreateSpringConstraint(i, ki, vj, k[j]));
